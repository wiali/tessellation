...
namespace tessellation
{
    ...
    template < class Base, template <typename> class A >
    class Arity1 : public A<Base > {};

    template < class Base, template <typename> class A, template <typename> class B >
    class Arity2 : public B<Arity1<Base, A> > {};

    template < class Base, template <typename> class A, template <typename> class B, template <typename> class C >
    ...
    template < class Base, template <typename> class A, template <typename> class B, template <typename> class C, 
        template <typename> class D, template <typename> class E, template <typename> class F >
    class Arity6 : public F<Arity5<Base, A, B, C, D, E> > {};

    template < class Base, template <typename> class A, template <typename> class B, template <typename> class C, 
        template <typename> class D, template <typename> class E, template <typename> class F, template <typename> class G >
    class Arity7 : public G<Arity6<Base, A, B, C, D, E, F> > {};

    template < class Base, class TA, template <typename, typename> class A >
    class MArity1 : public A< Base, TA >{};

    template < class Base, class TA, template <typename, typename> class A, class TB, template <typename, typename> class B >
    ...
    public:
        ...
        inline P3ScalarType dot(const Point3& p) const
        {
            return (*this) * p;
        }
    ...
    template<class P3ScalarType>
    P3ScalarType PSDist(const Point3<P3ScalarType>& p, const Point3<P3ScalarType>& v1,
                        const Point3<P3ScalarType>& v2, Point3<P3ScalarType>& q)
    {
    ...
    namespace tri
    {
        ...
        public:
            ...
            void UnMarkAll()
            {
                tessellation::tri::UnMarkAll(*m);
            }
            bool IsMarked(OBJ_TYPE* obj)
            {
                return (tessellation::tri::IsMarked(*m, obj));
            }
            void Mark(OBJ_TYPE* obj)
            {
                tessellation::tri::Mark(*m, obj);
            }
    ...
    public:
        ...
        inline const PointType& P0() const
        {
            return _p0;
        }

        inline const PointType& P1() const
        {
            return _p1;
        }
    ...
    template <class ScalarType>
    void SegmentPointSquaredDistance(const Segment3<ScalarType>& s, const Point3<ScalarType>& p,
        Point3< ScalarType >& closest, ScalarType& sqr_dist)
    {
    ...
    public:
        ...
        {
            return (tessellation::GridClosest<GridPtrType, OBJPOINTDISTFUNCTOR, OBJMARKER>(*this, 
                _getPointDistance, _marker, _p, _maxDist, _minDist, _closestPt));
        }
    };
    ...
    typename SPATIAL_INDEX::ObjPtr  GridClosest(SPATIAL_INDEX& Si, OBJPOINTDISTFUNCTOR _getPointDistance,
            OBJMARKER& _marker, const typename OBJPOINTDISTFUNCTOR::QueryType& _p_obj,
            ...
            typename SPATIAL_INDEX::CoordType& _closestPt)
    {
        ...
        do
        {
            ...
            if (!winner)
            {
                newradius = radius + Si.voxel.Norm();
            }
            else
            {
                newradius = _minDist;

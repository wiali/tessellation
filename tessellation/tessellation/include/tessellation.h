...
namespace tessellation
{
    ...
    {
    public:
        static bool process(const std::vector<float>& raw_vertices, const std::vector<int>& indices,
            const std::vector<float>& uvs, const std::vector<float>& normals, const QMatrix4x4& transform,
            const std::string& lowMeshFilename, const QString& heighMapFileName, int tessellationFactor)
        {
            if (tessellationFactor <= 1)
                return false;

            CMeshO mesh;
            MeshSpace::MeshIO<MeshSpace::MeshFileType::OBJ>::loadMesh(raw_vertices, indices, uvs, normals, transform, mesh);

            refineMesh(lowMeshFilename, heighMapFileName, tessellationFactor, mesh);

            return true;
        }

        static bool process(const std::string& lowMeshFilename, const QString& heighMapFileName, int tessellationFactor)
        {
            if (tessellationFactor <= 1)
                return false;

            CMeshO mesh;
            MeshSpace::MeshIO<MeshSpace::MeshFileType::OBJ>::loadMesh(lowMeshFilename, mesh);
    ...
    private:
        ...
        static void updateDataMask(CMeshO& mesh, int neededDataMask)
        {
        ...
        static void UpdateBoxAndNormals(CMeshO& mesh)
        {
        ...
        */
        ...
        static Point3f Barycentric(Point3f aV1, Point3f aV2, Point3f aV3, Point3f aP)
        {
        ...
        static void updateUVs(CMeshO& mesh, CMeshO& mesh_old)
        {
            ...
            for (auto i = mesh.vert.begin(); i != mesh.vert.end(); i++)
            {
                ...
                if (fabs(u) > 1 || fabs(v) > 1 || qFuzzyCompare(u, 0.0f) || qFuzzyCompare(v, 0.0f))
                {
        ...
        static bool refineMesh(const std::string& lowMeshFilename, const QString& heighMapFileName, int tessellationFactor, CMeshO& mesh)
        {
            CMeshO mesh_old;
            ...
            int iterations = tessellationFactor - 1;
            ...
            QImage imageHeight;
            imageHeight.load(heighMapFileName + ".jpg");
            ...
            return MeshSpace::MeshIO<MeshSpace::MeshFileType::OBJ>::writeMesh(lowMeshFilename, mesh);
        }
    };
}
